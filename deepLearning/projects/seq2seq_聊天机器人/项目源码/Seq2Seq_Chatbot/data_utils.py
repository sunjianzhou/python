...
def load_dictionary(): #加载已有的常用字字典
    with open(with_path(DICTIONARY_PATH), 'r', encoding='UTF-8') as fp:
        dictionary = [EOS, UNK, PAD, GO] + json.load(fp) 
        #json.load(fp)完后是一个列表，这个列表里存的是常用字字典里面的所有内容,然后和前面的列表拼接起来
        # EOS即代表编码结束以及解码开始,UNK代表未知的字，PAD即要打的padding
        index_word = OrderedDict()  #从后面看并不需要有序字典,字典即可
...
def generate_bucket_dbs(input_dir,output_dir,buckets,tolerate_unk=1):
    ...
    for dirpath, _, filenames in os.walk(input_dir):
        for filename in (x for x in sorted(filenames) if x.endswith('.db')): #sorted多余，还增加时间开销
    ...
    for db_path in db_paths:
        ...
        # 比SELECT COUNT(1)要快
        total = c.execute('''SELECT MAX(ROWID) FROM conversation;''').fetchall()[0][0] #fetchall之后拿到的是[(13700,)]
        ret = c.execute('''SELECT ask, answer FROM conversation;''')
        #能全部拿出来放到内存里？？？既然可以，那也没必要使用数据库了，唯一要考虑的即对应的问题。
        #从下面可以看出，显然一问 对 一答。那就完全可以使用文件来替代。
        wait_insert = []
            ...
            if is_valid(ask) and is_valid(answer):
                for i in range(len(buckets)):
                    ...
                    if len(ask) <= encoder_size and len(answer) < decoder_size:
                        ...
                        break #即跳出当前for循环，使得语句不会重复出现在多个bucket里
...
if __name__ == '__main__':
    ...
    # 来源数据库目录
    db_path = ''
    #先检测一下有没有传进来参数，如果有参数，则认为传进来的是db文件路径
    ...
    if not os.path.exists(target_path):
        ...
        shutil.rmtree(target_path) #递归删除文件目录
        os.makedirs(target_path) #创建目录
