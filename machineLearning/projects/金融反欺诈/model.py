...
#先用logistic进行训练和预测
df = pd.read_csv('./data/feature711.csv')
...
feature_importance = rf.feature_importances_#这里的rf是模型名，基本每个模型都有这个参数
# print(feature_importance)
# print(feature_importance.max())
feature_importance = 100.0*(feature_importance/feature_importance.max())
index = np.argsort(feature_importance)[-13:] #这里的13是最后只剩下13个（亚编码之前）主要的特征变量，不同项目需要调整
